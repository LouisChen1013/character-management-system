...
class Monster(AbstractCharacter):
    ...
    def get_details(self):
        """Returns the full description of the monster"""

        details = f"The monster (id: {self.id} ) is {self.get_monster_ai_difficulty()} {self.get_monster_type()}"
        return details

    def get_full_details(self):
        """Returns the full description of the monster"""

        full_details = f"The monster (id: {self.id} ) is {self.get_monster_ai_difficulty()} {self.get_monster_type()} with {self.health} health and {self.damage} damage, Position: X = {self.get_position()[0]} Y = {self.get_position()[1]}"
        return full_details
